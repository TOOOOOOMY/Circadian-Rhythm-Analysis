...
class visualizer:
    def __init__(
        ...
        if common_setting["moving_avrg_range"]:
            target_data = percentage_cal(moving_avrg(common_setting["moving_avrg_range"], ranged_data))
        else :
            target_data = percentage_cal(ranged_data)
        for key in target_data.columns:
            types_dict[col_posi_linker(key)].append(target_data[key])
        del types_dict[0]
    ...
    # MTとWTとかの系統単位での比較
    def strain_compare(self, strain_compare_info={}, graph_width = 4, graph_length = 4, col_number = 2, cal_range=6):
